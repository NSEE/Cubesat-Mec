function [Jstructure,Jmotor,Jflywheel,J,CGflywheel,flywheelmass] = inertia(proj,spec)
...
% Structure Inertia
    ...
    [Jstructure] = [(((proj.structure.mass)*(((proj.structure.length.side.width)^2)+((proj.structure.length.side.height)^2)))/12) 0 0;0 (((proj.structure.mass)*(((proj.structure.length.side.width)^2)+((proj.structure.length.side.height)^2)))/12) 0;0 0 (((proj.structure.mass)*(((proj.structure.length.side.width)^2)+((proj.structure.length.side.width)^2)))/12)];

% Motor Inertia

    [Jmotor] = [0 0 0;0 0 0;0 0 proj.motor.inertia];
...
% Flywheel Center of Gravity    
    ...
    flywheelmass = proj.flywheel.mass;
...
% Cubesat Inertia
    ...
    [J] = ((Jmotor) + (Jflywheel) + (Jstructure)); % [kg.m^2] - Inertia Cubesat
